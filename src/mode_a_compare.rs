...
pub fn run_detailed_compare(is_phrase_mode: bool, path_a: &str, path_b: &str) {
    ...
    println!("\x1b[1;37m{:>4} │ {:^7} │ {:<width$} │ {:<width$}\x1b[0m", 
             "行號", "狀態", "原始參考 (A)", "翻譯成果 (B)", width = COL_WIDTH);
    println!("{}", "━".repeat(115));
    ...
    for i in 0..std::cmp::max(lines_a.len(), lines_b.len()) {
        ...
        let expected_b = translate_single_line(&converter, &guard, &a, &current_section);

        if b == expected_b {
            println!("{:>4} │ \x1b[1;32m[ OK  ]\x1b[0m │ \x1b[2m{} │ {}\x1b[0m", 
                     line_num, truncate(&a, COL_WIDTH), truncate(&b, COL_WIDTH));
        } else {
            print!("{:>4} │ \x1b[1;31m[ ERR ]\x1b[0m │ ", line_num);
            print_github_diff(&expected_b, &b);
            println!();
        }
    }
}
