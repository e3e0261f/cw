pub mod report_format {
    ...
    pub enum ResultStatus { Success, VerifWarning, ConvertError }
    #[derive(Debug, Clone)]
    pub struct SubtitleIssue { pub line: usize, pub message: String }
    #[derive(Debug)]
    pub struct FileReport {
        pub input_name: String, pub output_name: String, pub temp_log_path: PathBuf,
        pub status: ResultStatus, pub issues: Vec<SubtitleIssue>,
        pub translated_pairs: Vec<(usize, String, String)>, pub duration: Duration,
    }
}
...
pub mod core {
    ...
    pub struct Config {
        pub discord_webhook: String, pub phrase_mode: bool, pub verbosity: u32,
        pub auto_discord: bool, pub log_directory: String, pub log_file_prefix: String,
        pub log_file_date_format: String, pub log_level: String, pub log_max_size_mb: u64,
        pub log_backup_count: u32, pub mention_id: String, pub discord_interval: u64,
        pub translate_error: bool, pub show_stats: bool, pub discord_show_errors: bool,
    ...
    impl Default for RawGuard { fn default() -> Self { Self::new() } }
    impl RawGuard {
        pub fn new() -> Self {
            Self {
        ...
        pub fn split_ass_line<'a>(&self, l: &'a str) -> (&'a str, &'a str) {
            let mut c = 0;
            for (i, ch) in l.char_indices() { if ch == ',' { c += 1; if c == 9 { return (&l[..i+1], &l[i+1..]); } } }
            (l, "")
        }
    }
    ...
    pub fn translate_single_line(conv: &OpenCC, guard: &RawGuard, l: &str, s: &str) -> String {
        if guard.is_forbidden_zone(l, s) || is_srt_structure(l) { return l.to_string(); }
        if (l.starts_with("Dialogue:") || l.starts_with("Comment:")) && s == "[Events]" {
            let (m, c) = guard.split_ass_line(l);
            let mut last = 0; let mut res = String::new();
    ...
    pub fn diagnose_file(path: &str, _translate: bool) -> Vec<SubtitleIssue> {
        ...
        let content = fs::read_to_string(path).unwrap_or_default().replace('\u{feff}', "");
    ...
    pub fn create_log(p_a: &str, p_b: &str, log_p: &PathBuf, status: &ResultStatus, _max: u64, _count: u32, issues: &[SubtitleIssue]) -> io::Result<()> {
        let mut f = OpenOptions::new().create(true).append(true).open(log_p)?;
        writeln!(f, "\n批次：{} | 原檔：{} | 成果：{}", Local::now().format("%Y-%m-%d %H:%M:%S"), p_a, p_b)?;
        for iss in issues { writeln!(f, "🛠️ L{:03} {}", iss.line, iss.message)?; }
        writeln!(f, "[ 狀態：{:?} ]\n{}", status, "-".repeat(40))?;
        Ok(())
    }
}
