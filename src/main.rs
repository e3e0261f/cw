...
fn main() -> std::io::Result<()> {
    ...
    if is_compare_mode {
        ...
        for (idx, path_str) in file_paths.iter().enumerate() {
            ...
            if fix_needed { v_errs.push("原檔不規範：末尾遺失空行。已修復。".to_string()); }
        ...
        if is_discord_mode && !config.discord_webhook.is_empty() && !reports.is_empty() {
            ...
            match mode_b_discord::execute(
                ...
                Ok(_) => println!("\x1b[1;32m ✔ 成功：所有訊息與附件已按順序送達。\x1b[0m"),
                Err(e) => ui_style::print_check_err(&format!("Discord 任務異常: {}", e)),
            }
        }
    }
    Ok(())
