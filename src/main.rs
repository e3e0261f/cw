...
fn main() -> std::io::Result<()> {
    ...
    if args.iter().any(|arg| arg == "--init") { return core::Config::generate_default(); }
    ...
    if is_a {
        if paths.len() >= 2 { mode_a_compare::run_detailed_compare(is_p, &paths[0], &paths[1]); }
    } else {
        println!("\n\x1b[1;36m🚀 CW 1.9.2 | 模式: {} | 日誌等級: {}\x1b[0m", if is_p { "S2TWP" } else { "S2T" }, config.log_level);
        ...
        for (idx, path_str) in paths.iter().enumerate() {
            ...
            match core::run_safe_translate(is_p, path_str, &format!("{}.tmp", path_str), fix) {
                Ok(pairs) => {
                    ...
                    let out_name = if is_d { fs::rename(format!("{}.tmp", path_str), path_str)?; path_str.to_string() } 
                                   else { let n = format!("{}.txt", path_str); let _ = fs::rename(format!("{}.tmp", path_str), &n); n };
                    let log_p = Path::new(&config.log_directory).join(format!("{}.log", config.log_file_prefix));
                    let _ = core::create_log(path_str, &out_name, &log_p, &ResultStatus::Success, config.log_max_size_mb, config.log_backup_count, &issues);
                    reports.push(FileReport { input_name: path_str.clone(), output_name: out_name, temp_log_path: log_p, status: ResultStatus::Success, issues, translated_pairs: pairs, duration: file_start.elapsed() });
        ...
        if is_b && !config.discord_webhook.is_empty() {
            let _ = mode_b_discord::execute(&config.discord_webhook, None, &config.mention_id, config.discord_interval, config.show_stats, config.discord_show_errors, &reports);
        }
    }
    Ok(())
}
...
fn run_stdin_mode(is_phrase: bool) {
    let config = if is_phrase { opencc_rust::DefaultConfig::S2TWP } else { opencc_rust::DefaultConfig::S2T };
