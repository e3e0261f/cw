...
pub fn execute(
    ...
    for (i, chunk) in chunks_to_send.iter().enumerate() {
        ...
        let resp = client.post(webhook_url).multipart(form).send().map_err(|e| e.to_string())?;
        println!("[發送中] 第 {}/{} 段 ... [HTTP {}]", i+1, chunks_to_send.len(), resp.status().as_u16());
